...
- a default 4:3 canvas for presentation-style drafts unless the figure grammar clearly needs another ratio
- generous outer margins with tighter internal spacing
- a large main subplot that carries the first-glance message
- panel-bound text should live inside its subplot whenever feasible
...
def draw_central_comparison():
    ...
    fig = plt.figure(figsize=(10.4, 7.8), facecolor="#f7f1e6")
    ...
    title_ax = fig.add_subplot(gs[4:12, 10:90])
    top = fig.add_subplot(gs[14:69, 10:90])
    ...
    fig.text(0.965, 0.03, "powered by SpotPaper", fontsize=8.8, color="#a9a29a", ha="right")
